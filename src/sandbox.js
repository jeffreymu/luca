...
export class DockerSandboxManager {
  ...
  async create({repoPath,image="node:22-alpine",network=false,memory="1g"}){
    if(!repoPath)throw new Error("repoPath required");
    const id=`lucapi-${randomUUID().slice(0,8)}`;
    const args=["run","-d","--name",id,"--label","lucapi.sandbox=true","--workdir","/workspace","--memory",memory,"--cpus","2","-v",`${repoPath}:/workspace`];
