...
try{
  await fetch(`${encryptedBase}/api/providers`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:"Encrypted",baseUrl:"http://example.test/v1",apiKey:"super-secret-key",model:"test",setActive:true})});
  const raw=new DatabaseSync(encryptedPath).prepare("SELECT api_key FROM providers").get().api_key;check("configured secret key encrypts credentials at rest",raw.startsWith("enc:v1:")&&!raw.includes("super-secret-key"));
